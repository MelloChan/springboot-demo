...
$(function () {
    ...
    $(grid_selector).jqGrid({
        url: "/infoOS/department/findByAll",
        datatype: "json",
        mtype: 'POST',
        height: window.screen.height - 550,
        colModel: [
            {label: 'id', name: 'id', width: 75, hidden: true},
            {label:'部门名称',name:'name',width:200},
            {label: '经理编号', name: 'eId', width: 200}
        ],
    ...
    $("#deleteDepartmentBtn").click(function () {
        ...
        if (rows.length > 0) {
            $.messager.confirm("温馨提示", "是否确定删除所选记录？", function () {
                $.ajax({
                    url: "/infoOS/department/deleteByIds",
                    cache: false,
                    type: "post",
                    data: {"ids": rows.join(",")},
                    beforeSend: function () {
...
function saveDepartment() {
    ...
    $.ajax({
        ...
        complete: function () {
            $('#saveDepartmentBtn').removeAttr("disabled");
        },
        error: function (data) {

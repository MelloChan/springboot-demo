...
$(function () {
    ...
    $(grid_selector).jqGrid({
        ...
        colModel: [
            ...
            {label: '工资', name: 'salary', width: 200},
            {label:'奖金',name:'bonus',width:200},
            {label:'罚金',name:'punish',width:200}
    ...
    //修改工资信息，弹出修改窗
    $("#modifySalaryBtn").click(function () {
        ...
        if (rows == 0) {
            ...
            $("#punish").val(data.punish);
            $('#myModalLabel').text('修改工资信息');
            $("#addModal").modal({
                keyboard: false,
                show: true,
                backdrop: "static"
...
function saveSalary() {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            // 禁用按钮防止重复提交
            $('#saveSalaryBtn').attr({disabled: "disabled"});
        },
        success: function (result) {
            if (result.flag === true) {
        ...
        complete: function () {
            $('#saveSalaryBtn').removeAttr("disabled");
        },
        error: function (data) {

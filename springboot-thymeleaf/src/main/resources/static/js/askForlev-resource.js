...
$(function () {
    ...
    $(grid_selector).jqGrid({
        url: "/infoOS/askForlev/findByAll",
        datatype: "json",
        mtype: 'POST',
        ...
        colModel: [
            ...
            {label: '请假天数', name: 'day', width: 200},
            {label: '请假原因', name: 'reason', width: 200},
            {label: '请假时间', name: 'date', width: 200}
        ],
    ...
    //新增与请假信息，弹出新增窗口
    $("#addAskForlevBtn").click(function () {
        task = "saveAskForlev";
        initData();
        $('#myModalLabel').text('新增请假信息');
        $("#addModal").modal({
            keyboard: false,
            show: true,
            backdrop: "static"
    ...
    //保存信息
    $('#saveAskForlevBtn').click(function () {
        saveAskForlev();
    });

    //修改请假信息，弹出修改窗
    $("#modifyAskForlevBtn").click(function () {
    ...
    $("#deleteAskForlevBtn").click(function () {
        var rows = $(grid_selector).getGridParam('selarrrow');
        if (rows.length > 0) {
            $.messager.confirm("温馨提示", "是否确定删除所选记录？", function () {
                $.ajax({
                    url: "/infoOS/askForlev/deleteByIds",
                    cache: false,
                    type: "post",
                    data: {"ids": rows.join(",")},
                    beforeSend: function () {
...
function saveAskForlev() {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            // 禁用按钮防止重复提交
            $('#saveAskForlevBtn').attr({disabled: "disabled"});
        },
        success: function (result) {
            if (result.flag === true) {
        ...
        complete: function () {
            $('#saveAskForlevBtn').removeAttr("disabled");
        },
        error: function (data) {

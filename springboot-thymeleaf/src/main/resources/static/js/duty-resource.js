...
$(function () {
    ...
    $(grid_selector).jqGrid({
        ...
        colModel: [
            ...
            {label: '出勤天数', name: 'attend', width: 200},
            {label:'早退次数',name:'leave',width:200},
            {label:'迟到次数',name:'late',width:200},
            {label:'旷班次数',name:'absent',width:200}
    ...
    $("#deleteDutyBtn").click(function () {
        var rows = $(grid_selector).getGridParam('selarrrow');
        if (rows.length > 0) {
            $.messager.confirm("温馨提示", "是否确定删除所选记录？", function () {
                $.ajax({
                    url: "/infoOS/duty/deleteByIds",
                    cache: false,
                    type: "post",
                    data: {"ids": rows.join(",")},
                    beforeSend: function () {
...
function saveDuty() {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            // 禁用按钮防止重复提交
            $('#saveDutyBtn').attr({disabled: "disabled"});
        },
        success: function (result) {
            if (result.flag === true) {
        ...
        complete: function () {
            $('#saveDutyBtn').removeAttr("disabled");
        },
        error: function (data) {

...
$(function () {
    ...
    $(grid_selector).jqGrid({
        url: "/infoOS/admin/findByAll",
        datatype: "json",
        mtype: 'POST',
        height: window.screen.height - 550,
        colModel: [
            {label: 'id', name: 'id', width: 75, hidden: true},
            {label: '用户', name: 'username', width: 200},
            {label: '密码', name: 'password', width: 200}
        ],
...
function saveAdmin() {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            // 禁用按钮防止重复提交
            $('#saveAdminBtn').attr({disabled: "disabled"});
        },
        success: function (result) {
            if (result.flag === true) {
        ...
        complete: function () {
            $('#saveAdminBtn').removeAttr("disabled");
        },
        error: function (data) {
...
function refreshData() {
    $(grid_selector).jqGrid('setGridParam', {
        postData: {username: null, password: null},
        page: 1
    }).trigger("reloadGrid");
}
